...
echo "--- Uptime Kuma 数据恢复 ---"
echo "------------------------------"
echo "警告：在运行此脚本前，请确保 Uptime Kuma 进程已停止！"
echo "例如：pm2 stop uptime-kuma"
echo "---------------"
echo "预备工作："
echo "先将压缩包“data.tar.gz”(默认使用docker数据，实际上是通用的)上传到root文件夹下"
# read -p "准备好后回车确认"
echo "------------------------------"
echo "开始迁移数据…"
...
TARGET_DIR="/root/uptime-kuma/data"                  # Uptime Kuma 数据路径
...
if [ ! -d "$TARGET_DIR" ]; then
    ...
    echo "请确认 Uptime Kuma 目标数据目录已存在或已准备好。" # 修改了提示信息
    exit 1
fi
...
echo "--- 数据恢复完成！ ---"
echo "现在你可以重新启动 Uptime Kuma 服务以加载新数据了。"
